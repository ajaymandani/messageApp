        ...
        authListner = Auth.auth().addStateDidChangeListener({ auth, user in
            ...
            if user != nil && UserDefaults.standard.object(forKey: kcurrentUser) != nil{
                DispatchQueue.main.async {
                    let storybaord = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "mainView") as! UITabBarController
                    self.window?.rootViewController = storybaord
                }
            }
        })
    }
